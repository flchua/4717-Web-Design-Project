...
<body class="debug o f d">
    <?php
        ...
        // //Get popular items
        // $query = "SELECT p.id, p.name, p.price, p.discount, COUNT(*) AS numberOfTimesBought FROM products AS p, inventory AS i, orders_inventory AS oi 
        //             WHERE i.id = oi.inventoryID AND p.id = i.productsID 
        //             GROUP BY(p.id) 
